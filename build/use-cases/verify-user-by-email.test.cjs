...
var InMemoryUserRepository = class {
  ...
  validate(email, date) {
    return __async(this, null, function* () {
      const index2 = this.data.findIndex((value) => value.email === email);
      this.data[index2].validated_at = date;
...
var ErrorPlugin = {
  ...
  serialize(val, config2, indentation, depth, refs, printer2) {
    ...
    refs = [...refs, val];
    const hitMaxDepth = ++depth > config2.maxDepth;
    const _a2 = val, { message, cause } = _a2, rest = __objRest(_a2, ["message", "cause"]);
...
function stringify(object2, maxDepth = 10, _a2 = {}) {
  ...
  return result.length >= MAX_LENGTH && maxDepth > 1 ? stringify(object2, Math.floor(Math.min(maxDepth, Number.MAX_SAFE_INTEGER) / 2), __spreadValues({ maxLength }, options)) : result;
}
...
function diffLinesRaw(aLines, bLines, options) {
  var _a2, _b;
  const truncate3 = (_a2 = options == null ? void 0 : options.truncateThreshold) != null ? _a2 : false;
  const truncateThreshold = Math.max(
    Math.floor((_b = options == null ? void 0 : options.truncateThreshold) != null ? _b : 0),
    0
  );
...
function C2(e, t, n2) {
  ...
  let g = (v) => {
    let _a2 = a3 || {
      configurable: true,
      writable: true
    }, { value: M } = _a2, h3 = __objRest(_a2, ["value"]);
...
function getObjectKeys(object2) {
  return [
    ...
    ...Object.getOwnPropertySymbols(object2).filter(
      (s2) => {
        var _a2;
        return (_a2 = Object.getOwnPropertyDescriptor(object2, s2)) == null ? void 0 : _a2.enumerable;
      }
    )
  ];
}
...
function createTestContext(test5, runner2) {
  ...
  context.skip = (note) => {
    var _a3;
    (_a3 = test5.result) != null ? _a3 : test5.result = { state: "skip" };
  ...
  context.onTestFailed = (handler, timeout) => {
    ...
    test5.onFailed.push(
      withTimeout(handler, timeout != null ? timeout : runner2.config.hookTimeout, true)
    );
  };
  ...
  const fixtureArray = Object.entries(fixtures).map(
    ([prop, value]) => {
      ...
      if (Array.isArray(value) && value.length >= 2 && isObject(value[1]) && Object.keys(value[1]).some((key) => fixtureOptionKeys.includes(key))) {
        ...
        fixtureItem.value = fixtureItem.injected ? (_a2 = inject2(prop)) != null ? _a2 : userValue : userValue;
      }
...
function createSuiteCollector(name, factory = () => {
  ...
  const task = function(name2 = "", options = {}) {
    ...
    const task2 = {
      id: "",
      name: name2,
      suite: (_a2 = collectorContext.currentSuite) == null ? void 0 : _a2.suite,
      each: options.each,
      fails: options.fails,
      context: void 0,
      type: "test",
      file: void 0,
      retry: (_a3 = options.retry) != null ? _a3 : runner.config.retry,
    ...
    if (runner.config.includeTaskLocation) {
      ...
      Error.stackTraceLimit = limit;
      const stack = findTestFileStackTrace(error, (_d = task2.each) != null ? _d : false);
      if (stack) {
  ...
  const test22 = createTest(function(name2, optionsOrFn, optionsOrTest) {
    ...
    const test32 = task(formatName(name2), __spreadProps(__spreadValues(__spreadValues({}, this), options), {
      handler
    }));
    test32.type = "test";
  });
  ...
  function initSuite(includeLocation) {
    ...
    if (runner && includeLocation && runner.config.includeTaskLocation) {
      ...
      const error = new Error("stacktrace").stack;
      Error.stackTraceLimit = limit;
      const stack = findTestFileStackTrace(error, (_a3 = suite2.each) != null ? _a3 : false);
      if (stack) {
...
function createSuite() {
  function suiteFn(name, factoryOrOptions, optionsOrFactory) {
    ...
    options = __spreadProps(__spreadValues(__spreadValues({}, currentSuite == null ? void 0 : currentSuite.options), options), {
      shuffle: (_c = (_b = (_a3 = this.shuffle) != null ? _a3 : options.shuffle) != null ? _b : (_a2 = currentSuite == null ? void 0 : currentSuite.options) == null ? void 0 : _a2.shuffle) != null ? _c : runner == null ? void 0 : runner.config.sequence.shuffle
    });
...
function createTaskCollector(fn2, context) {
  ...
  taskFn.extend = function(fixtures) {
    const _context = mergeContextFixtures(
      ...
      (key) => {
        var _a2, _b;
        return (_b = (_a2 = getRunner()).injectValue) == null ? void 0 : _b.call(_a2, key);
      }
    );
...
function beforeEach(fn2, timeout = getDefaultHookTimeout()) {
  ...
  return getCurrentSuite().on(
    "beforeEach",
    Object.assign(
      withTimeout(withFixtures(fn2), timeout != null ? timeout : getDefaultHookTimeout(), true),
...
var onTestFailed = createTestHook(
  ...
  (test5, handler, timeout) => {
    test5.onFailed || (test5.onFailed = []);
    test5.onFailed.push(
      withTimeout(handler, timeout != null ? timeout : getDefaultHookTimeout(), true)
    );
  }
);
...
function saveInlineSnapshots(environment, snapshots) {
  return __async(this, null, function* () {
    ...
    yield Promise.all(
      Array.from(files).map((file) => __async(this, null, function* () {
...
function saveRawSnapshots(environment, snapshots) {
  return __async(this, null, function* () {
    yield Promise.all(
      snapshots.map((snap) => __async(this, null, function* () {
        if (!snap.readonly) {
...
var SnapshotState = class _SnapshotState {
  ...
  static create(testFilePath, options) {
    return __async(this, null, function* () {
      const snapshotPath = yield options.snapshotEnvironment.resolvePath(
        testFilePath
      );
      const content = yield options.snapshotEnvironment.readSnapshotFile(
        snapshotPath
      );
...
${JSON.stringify(
      ...
      this._inlineSnapshotStacks.push(__spreadProps(__spreadValues({}, stack), { testId, snapshot: receivedSerialized }));
    }
  ...
  setup(filepath, options) {
    return __async(this, null, function* () {
      ...
      this.snapshotStateMap.set(
        filepath,
        yield SnapshotState.create(filepath, options)
      );
    });
  }
  finish(filepath) {
  ...
  assertRaw(options) {
    return __async(this, null, function* () {
      ...
      if (rawSnapshot.content == null) {
        ...
        rawSnapshot.file = yield snapshotState.environment.resolveRawPath(
          filepath,
          rawSnapshot.file
        );
        rawSnapshot.content = (_a2 = yield snapshotState.environment.readSnapshotFile(rawSnapshot.file)) != null ? _a2 : void 0;
      }
...
// node_modules/vitest/dist/chunks/vi.B5EKKJdE.js
var unsupported = [
...
function createExpectPoll(expect2) {
  return function poll(fn2, options = {}) {
    var _a2, _b, _c, _d;
    const state = getWorkerState();
    const defaults = (_b = (_a2 = state.config.expect) == null ? void 0 : _a2.poll) != null ? _b : {};
    const {
      interval = (_c = defaults.interval) != null ? _c : 50,
      timeout = (_d = defaults.timeout) != null ? _d : 1e3,
      message
    } = options;
    ...
    const proxy = new Proxy(assertion, {
      get(target, key, receiver) {
        ...
        return function(...args) {
          ...
          const promise = () => new Promise((resolve4, reject) => {
            ...
            const check = () => __async(this, null, function* () {
              try {
                ...
                resolve4(yield assertionFunction.call(assertion, ...args));
          ...
          let awaited = false;
          (_a3 = test5.onFinished) != null ? _a3 : test5.onFinished = [];
          test5.onFinished.push(() => {
            if (!awaited) {
...
var SnapshotPlugin = (chai2, utils) => {
  ...
  utils.addMethod(
    ...
    function __INLINE_SNAPSHOT__(properties, inlineSnapshot, message) {
      ...
      const isInsideEach = test5.each || ((_a2 = test5.suite) == null ? void 0 : _a2.each);
      if (isInsideEach) {
        throw new Error(
...
function createExpect(test5) {
  ...
  setState(
    __spreadProps(__spreadValues({}, globalState), {
      assertionCalls: 0,
...
function createVitest() {
  ...
  const utils = {
    useFakeTimers(config2) {
      ...
      if (config2) {
        timers().configure(__spreadValues(__spreadValues({}, workerState.config.fakeTimers), config2));
      } else {
...
// src/use-cases/verify-user-by-email.test.ts
...
describe("Verify user by email use case", () => {
  ...
  test3("should be able generate a token and send verification email", () => __async(void 0, null, function* () {
    yield inMemoryRepository.create({
      name: "test-name",
      email: "test@gmail.com",
      password_hash: yield (0, import_bcryptjs.hash)("123456", 6)
    });
    const mockSendVeriicationEmail = vi.mocked(sendVerificationEmail).mockResolvedValue(void 0);
    const { token } = yield sut.execute({
      email: "test@gmail.com"
    });
    globalExpect(token).toEqual(globalExpect.any(String));
    globalExpect(mockSendVeriicationEmail).toHaveBeenCalledWith(
      "test@gmail.com",
      token
    );
  }));
  test3("should not be able send email if user is realdy validated", () => __async(void 0, null, function* () {
    const user = yield inMemoryRepository.create({
      name: "test-name",
      email: "test@gmail.com",
      password_hash: yield (0, import_bcryptjs.hash)("123456", 6),
      validated_at: /* @__PURE__ */ new Date()
    });
    yield globalExpect(sut.execute({ email: user.email })).rejects.toBeInstanceOf(
      UserAlreadyValidatedError
    );
  }));
  test3("should throw UnableSendEmailError if email sending fails", () => __async(void 0, null, function* () {
    yield inMemoryRepository.create({
      name: "test-name",
      email: "test@gmail.com",
      password_hash: yield (0, import_bcryptjs.hash)("123456", 6)
