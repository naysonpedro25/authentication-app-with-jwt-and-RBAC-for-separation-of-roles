...
// src/use-cases/change-user-password.test.ts
...
describe("Change user password use case", () => {
  ...
  test3("should be able change user password", () => __async(void 0, null, function* () {
    ...
    const { user } = yield sut.execute({
      ...
      password: "123456",
      newPassword: "z\xE9pelin"
    });
    const isEqualsPasswords = yield (0, import_bcryptjs2.compare)("z\xE9pelin", user.password_hash);
    globalExpect(isEqualsPasswords).toEqual(true);
    globalExpect(user.password_hash).not.toEqual(oldPassword);
  }));
});
